...
def summary_text(decoded: dict[str, Any]) -> str:
    ...
    if decoded.get("acas_vs") is not None:
        parts.append(f"положение воздух/земля: {decoded['acas_vs']}")
    if decoded.get("acas_sl") is not None:
        parts.append(f"текущий уровень чувствительности ACAS: {decoded['acas_sl']}")
    if decoded.get("acas_ri") is not None:
        parts.append(f"информация ответа «воздух–воздух»: {decoded['acas_ri']}")
    if decoded.get("acas_cc") is not None and decoded.get("df") == 0:
        parts.append(f"возможность кросс-линка: {decoded['acas_cc']}")
    if decoded.get("acas_ra") is not None:
        parts.append(f"действующие конфликтные ситуации: {decoded['acas_ra']}")
    if decoded.get("acas_rac") is not None:
        parts.append(f"дополнения к RA: {decoded['acas_rac']}")
    if decoded.get("acas_rat") is not None:
        parts.append(f"индикатор прекращения RA: {decoded['acas_rat']}")
